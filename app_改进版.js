...
async function streamDongpoAnswer(text,bot){var system=dongpoPersona+'\n\nreference:\n'+kbSummary();var messages=[{role:'system',content:system}].concat(askTurns.slice(-6),[{role:'user',content:CATEGORY_PREFIX[askCategory]+'\n'+text}]);var res=await fetch(API_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:messages})});if(!res.ok)throw new Error('api failed');var log=document.getElementById('ask-log');var type=res.headers.get('Content-Type')||'';if(!res.body||type.indexOf('application/json')!==-1){var data=await res.json();bot.textContent=(data&&data.content)||'';log.scrollTop=log.scrollHeight;if(!bot.textContent)throw new Error('empty');return;}var reader=res.body.getReader();var decoder=new TextDecoder();var got=false;bot.textContent='';while(true){var result=await reader.read();if(result.done)break;var chunk=decoder.decode(result.value,{stream:true});if(!chunk)continue;got=true;bot.textContent+=chunk;log.scrollTop=log.scrollHeight;}var tail=decoder.decode();if(tail){got=true;bot.textContent+=tail;log.scrollTop=log.scrollHeight;}if(!got||!bot.textContent.trim())throw new Error('empty');}
