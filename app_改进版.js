...
let narrationList=[]; let currentNarrationAudio=null; let dongpoPersona=''; let dongpoKb=null; let askCategory='life'; let askTurns=[]; let askBusy=false; let currentQuickPrompt=''; let tourIndex=0; let tourActive=false; const TOUR_DONE_KEY='sushi_tour_done_20260505_digital_guide3';
const tourSteps=[
...
function nextTourStep(){if(tourIndex>=tourSteps.length-1){finishTour();return;}tourIndex+=1;showTourStep();}function finishTour(){tourActive=false;localStorage.setItem(TOUR_DONE_KEY,'1');document.getElementById('tour-shade').classList.remove('on');}function clearTour(){tourActive=false;document.getElementById('tour-shade').classList.remove('on');}
function start(){
