...
function updateAskHint(){var p=askPrompts[Math.floor(Math.random()*askPrompts.length)];currentQuickPrompt=p;document.getElementById('ask-hint').textContent='\u5feb\u6377\u63d0\u95ee\uff1a'+p;document.getElementById('ask-input').placeholder=p;}function appendAskMessage(role,text){var log=document.getElementById('ask-log');var el=document.createElement('div');el.className='ask-msg '+(role==='user'?'user':'bot');el.textContent=text;log.appendChild(el);log.scrollTop=log.scrollHeight;return el;}
async function sendAskMessage(forcedText){if(askBusy)return;var input=document.getElementById('ask-input');var text=(forcedText||input.value).trim();if(!text)return;if(!forcedText)input.value='';appendAskMessage('user',text);askTurns.push({role:'user',content:text});var bot=appendAskMessage('bot','');askBusy=true;try{var key=getDongpoApiKey();if(!key)throw new Error('no api key');await streamDongpoAnswer(text,bot,key);}catch(e){bot.textContent=fallbackReplies[Math.floor(Math.random()*fallbackReplies.length)];}finally{askTurns.push({role:'assistant',content:bot.textContent});askTurns=askTurns.slice(-8);askBusy=false;}}
